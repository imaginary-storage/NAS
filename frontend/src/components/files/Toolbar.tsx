      ...
      <div className="flex flex-1 flex-wrap items-center gap-2">
        <div className="relative min-w-[220px] flex-1 lg:max-w-sm">
          ...
          <Input
            ...
            className="h-9 border-slate-200 bg-slate-50 pl-9 pr-9 text-sm focus-visible:bg-white dark:border-slate-700 dark:bg-white/5 dark:focus-visible:bg-white/10"
          ...
          {searchQuery && (
            <button
              ...
              className="absolute right-2 top-1/2 flex h-6 w-6 -translate-y-1/2 items-center justify-center rounded-md text-slate-400 transition hover:bg-slate-200 hover:text-slate-600 dark:hover:bg-slate-700 dark:hover:text-slate-300"
          ...
          <>
            {hasSelection && (
              <>
                ...
                <Button variant="ghost" size="sm" onClick={onBulkRestore} className="text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-white/10">
            ...
            {!hasSelection && (
              <Button variant="ghost" size="sm" onClick={onEmptyTrash} className="text-red-500 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-500/10">
      ...
      <div className="flex items-center justify-end gap-1">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="sm" className="text-slate-500 hover:bg-slate-100 hover:text-slate-700 dark:text-slate-400 dark:hover:bg-white/10 dark:hover:text-slate-200">
        ...
        {viewMode === 'grid' && (
          <div className="flex items-center gap-0.5 rounded-lg border border-slate-200 p-0.5 dark:border-slate-700">
            <button
              ...
              disabled={iconSize <= 1}
              className="rounded-md p-1.5 text-slate-400 transition-all hover:text-slate-600 disabled:opacity-30 dark:hover:text-slate-200"
              aria-label="Decrease icon size"
            >
              <Minus className="h-3.5 w-3.5" />
            </button>
            <span className="min-w-[1.25rem] text-center text-xs font-medium text-slate-500 dark:text-slate-400">{iconSize}</span>
            <button
              onClick={() => dispatch(setIconSize(iconSize + 1))}
              disabled={iconSize >= 4}
              className="rounded-md p-1.5 text-slate-400 transition-all hover:text-slate-600 disabled:opacity-30 dark:hover:text-slate-200"
        ...
        <div className="ml-1 flex rounded-lg border border-slate-200 p-0.5 dark:border-slate-700">
          <button
            ...
            className={`rounded-md p-1.5 transition-all ${viewMode === 'grid' ? 'bg-indigo-50 text-indigo-600 shadow-sm dark:bg-indigo-500/15 dark:text-indigo-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'}`}
          >
            <Grid3x3 className="h-4 w-4" />
          </button>
          <button
            onClick={() => dispatch(setViewMode('list'))}
            className={`rounded-md p-1.5 transition-all ${viewMode === 'list' ? 'bg-indigo-50 text-indigo-600 shadow-sm dark:bg-indigo-500/15 dark:text-indigo-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'}`}
          >
